...
### 8.1 后端部署
- **环境要求**：Python 3.7+, SQLite 3.0+, Redis 7.0+ (可选)
- **安装步骤**：克隆代码仓库，创建虚拟环境，安装依赖，初始化数据库，启动服务
- **配置文件**：config.yaml，包含管理员配置、数据库配置、限流配置、加密配置、超时配置、密钥管理配置、缓存配置、日志配置

### 8.2 前端部署
- **环境要求**：Node.js 16+
- **安装步骤**：克隆代码仓库，安装依赖，配置环境变量，构建项目，部署静态文件

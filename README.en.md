...
**Lightweight LLM API Aggregation & Credit Billing Gateway · 轻量级 LLM API 聚合计费网关**

Turn idle API quota into revenue. One unified OpenAI-compatible endpoint for every LLM vendor.  
把闲置的 API 额度变成收益，一个接口接入所有大模型厂商。
...
> **Keywords**: LLM gateway · API aggregation · credit billing · key pool routing · OpenAI compatible · LLM proxy · AI API hub  
> **关键词**：LLM 网关 · API 聚合 · 积分计费 · 密钥池路由 · OpenAI 兼容 · 大模型代理
...
# Generate config.yaml (first-time setup)
# SQLite mode (local dev, no external DB required)
python3 scripts/init/init_config.py --sqlite
# MySQL mode (production, set env vars first — see Configuration)
# python3 scripts/init/init_config.py

# Initialize database
# SQLite:
python3 scripts/init/init_db.py
# MySQL: create the database first, then run:
...
├── scripts/
│   └── init/
│       ├── init_config.py  # Generate config.yaml (SQLite / MySQL mode)
│       ├── init_db.py      # Initialize SQLite DB & default user
│       └── init_mysql.sql  # MySQL DDL script (idempotent)
├── tests/              # Unit & regression tests
├── config.yaml         # Configuration file (generated by init_config.py)
└── requirements.txt
```
